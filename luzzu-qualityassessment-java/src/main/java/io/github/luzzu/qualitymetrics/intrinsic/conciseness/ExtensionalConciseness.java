package io.github.luzzu.qualitymetrics.intrinsic.conciseness;
...
/**
 * @author Santiago Londono Provides a measure of the redundancy of the dataset
 *         at the data level, by calculating the Extensional Conciseness metric,
...
public class ExtensionalConciseness extends AbstractQualityMetric {
	...
	/**
	 * MapDB database, used to persist the Map containing the instances found to be
	 * declared in the dataset
	 */
	private DB mapDB = MapDbFactory.createFilesystemDB();

	/**
	 * Map indexing the subjects detected during the computation of the metric.
	 * Every subject is identified by a different id (URI), which serves as key of
	 * the map. The value of each subject consists of a resource. A
	 * ConcurrentHashMap is used since it is synchronized and metric instances ought
	 * to be thread safe.
	 */
	private HTreeMap<String, ComparableSubject> pMapSubjects = this.mapDB.createHashMap("extensional-conciseness-map")
			.make();

	private List<Quad> problemList = new ArrayList<Quad>();

	/**
	 * Re-computes the value of the Extensional Conciseness Metric, by considering a
	 * new quad provided.
	 * 
	 * @param quad
	 *            The new quad to be considered in the computation of the metric.
	 *            Must be not null.
	 */

	public void compute(Quad quad) {
		if (quad.getSubject() != null && quad.getSubject().isURI() && quad.getPredicate() != null
				&& quad.getPredicate().isURI()) {
			// Every time a new quad is considered, check whether the subject has already
			// been identified
			...
			// Add or update the property stated by the current quad into the subject, as a
			// predicate with a value.
	...
	/**
	 * Returns the current value of the Extensional Conciseness Metric, computed as
	 * the ratio of the Number of Unique Subjects to the Total Number of Subjects.
	 * Subjects are the objects being described by the quads provided on invocations
	 * to the compute method, each subject is identified by its URI (the value of
	 * the subject attribute of the quad). Uniqueness of subjects is determined from
	 * its properties: one subject is said to be unique if and only if there is no
	 * other subject equivalent to it. - Note that two equivalent subjects may have
	 * different ids (URIs).
	 * 
	 * @return Current value of the Extensional Conciseness Metric: (No. of Unique
	 *         Subjects / Total No. of Subjects)
	 */
	...
	/**
	 * @author Santiago Londono Represents a subject (i.e. a resource), that is
	 *         described by a set of properties. Furthermore, provides the
	 *         functionality necessary to determine the equivalence of subjects
	 *         according to their properties. - Notice: this class is not thread
	 *         safe (since HashMaps are not synchronized).
	 */
	private static class ComparableSubject implements Serializable {
		...
		/**
		 * Map containing the properties that describe this subject. Maps allow
		 * properties to be retrieved by URI efficiently, which is quite convenient when
		 * comparing the sets of properties of subjects. HashMap is used, since
		 * instances of this class ought not to be thread safe.
		 */
		 ...
		 * 
		 * @param predicateUri
		 *            URI of the predicate corresponding to the property to be added
		 * @param objectValue
		 *            Value of the property to be added
		 */
		public void addProperty(String predicateUri, Node objectValue) {
			// To keep the memory footprint low, store in the properties hash a string
			// representation of the node value.
			// Node.toString() is suitable, as it "Answers a human-readable representation
			// of the Node" (see javadoc for Node class)
		 ...
		 * Two subjects are equivalent if they have the same set of properties, all with
		 * the same values (but not necessarily the same ids).
		 * 
		 * @param subject
		 *            Subject this instance will be compared with
		 * @return true if the parameter is not null and if both subjects are
