package io.github.luzzu.qualitymetrics.utilities.mapdb;
...
/**
 * Creates and properly sets up instances of mapdb databases, stored in the
 * filesystem.
 * 
 * @author slondono
 *
...
public class MapDbFactory {
	 ...
	 * 
	 * @param mapDbDirectory
	 *            Path of the directory to store all the mapdb files
	 */
	...
	public static DB createFilesystemDB() {
		...
		DB mapDB = DBMaker.newFileDB(new File(mapDbDir + "mapdb-" + timestamp)).mmapFileEnableIfSupported()
				.closeOnJvmShutdown().deleteFilesAfterClose().transactionDisable().asyncWriteFlushDelay(2000)
				.cacheDisable().make();
	...
	public static DB createAsyncFilesystemDB() {
		...
		DB mapDB = DBMaker.newFileDB(new File(mapDbDir + "mapasyncdb-" + timestamp)).mmapFileEnableIfSupported()
				.closeOnJvmShutdown().deleteFilesAfterClose().transactionDisable().asyncWriteFlushDelay(2000)
				// .asyncWriteEnable()
				.cacheDisable().make();
